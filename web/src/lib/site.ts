export const SITE = {
  name: "Promise Proper Development Ltd.",
  nameBn: "প্রমিজ প্রপার ডেভেলপমেন্ট লিমিটেড",
  shortName: "Promise PPD",
  tagline: "প্রতিশ্রুতি · বিশ্বাস · উন্নয়ন",
  description:
    "ঢাকায় আবাসিক, বাণিজ্যিক ও বিনিয়োগযোগ্য সম্পত্তির জন্য আপনার বিশ্বস্ত অংশীদার।",
  phone: "+8801910065136",
  phoneDisplay: "+৮৮০ ১৯১০-০৬৫১৩৬",
  whatsapp: "8801910065136",
  email: "promisegroup2023@gmail.com",
  address:
    "১২৩/১/২ কাজী টাওয়ার, দক্ষিণ যাত্রাবাড়ী, ঢাকা-১২০৪, বাংলাদেশ",
  hours: "মঙ্গল – রবি · সকাল ৯টা – সন্ধ্যা ৬টা",
  weeklyOff: "সাপ্তাহিক ছুটি: সোমবার",
  facebook: "https://www.facebook.com/profile.php?id=61557334089998",
  founded: 2010,
  socials: {
    facebook: "https://www.facebook.com/profile.php?id=61557334089998",
    youtube: "https://www.youtube.com/@promisepd",
    whatsapp: "https://wa.me/8801910065136",
    tiktok: "https://www.tiktok.com/@promisepd",
    instagram: "https://www.instagram.com/promisepd",
  },
  credit: {
    label: "Made by Mustaqeem Billah",
    url: "https://growthency.com/",
  },
};

export const HERO_PITCHES = [
  {
    name: "প্রমিজ সিটি",
    pitch: "ভাড়ার ঘরে স্বপ্ন দেখা শেষ — এবার নিজের একটা ঠিকানা।",
    color: "red",
  },
  {
    name: "আহবাব রিয়েল এস্টেট",
    pitch: "প্রতিটি ইট গাঁথা হয় আপনার বিশ্বাসের উপর।",
    color: "blue",
  },
  {
    name: "প্রমিজ ইন্টারন্যাশনাল",
    pitch: "ব্যাংক টাকা রাখে — আমরা টাকাকে কাজে লাগাই।",
    color: "indigo",
  },
  {
    name: "আহবাব ট্রাভেলস",
    pitch: "আল্লাহর ঘরে যাত্রা — আমরা পাশে হাঁটি প্রতিটি কদমে।",
    color: "amber",
  },
  {
    name: "ইন্টেরিয়র ও 3D ডিজাইন",
    pitch: "চার দেয়ালে আপনার গল্প — আমরা শিল্প করে তুলি।",
    color: "rose",
  },
];

export type Division = {
  slug: string;
  nameEn: string;
  nameBn: string;
  shortBn: string;
  tagline: string;
  description: string;
  longDescription: string;
  icon: string;
  accent: "red" | "blue" | "rose" | "indigo" | "amber";
  features: { title: string; description: string }[];
  highlights: string[];
};

export const DIVISIONS: Division[] = [
  {
    slug: "promise-city",
    nameEn: "Promise City",
    nameBn: "প্রমিজ সিটি",
    shortBn: "প্রমিজ সিটি",
    tagline: "রিয়েল এস্টেট, ফ্ল্যাট, জমি ও ল্যান্ডিং প্রপার্টি",
    description:
      "ঢাকার বুকে আপনার স্বপ্নের ঠিকানা — ফ্ল্যাট, জমি, ও ল্যান্ডিং প্রপার্টি ব্যবসা।",
    longDescription:
      "প্রমিজ সিটির অধীনে আমরা রিয়েল এস্টেট, ফ্ল্যাট, জমি এবং ল্যান্ডিং প্রপার্টি ব্যবসা পরিচালনা করি। আহবাব প্যালেস ১ ও ২, ফুজলা টাওয়ার, ফুজলা কমপ্লেক্স — সব আমাদের প্রমিজ সিটির অধীনে।",
    icon: "Building2",
    accent: "red",
    features: [
      {
        title: "প্রিমিয়াম লোকেশনে ফ্ল্যাট",
        description:
          "ঢাকার বিকাশমান এলাকায় বাছাইকৃত প্রকল্পে আধুনিক অ্যাপার্টমেন্ট।",
      },
      {
        title: "যাচাইকৃত জমি ও প্লট",
        description:
          "সম্পূর্ণ আইনি যাচাইসহ প্রিমিয়াম প্লট এবং ল্যান্ডিং প্রপার্টি।",
      },
      {
        title: "নমনীয় কিস্তি",
        description:
          "আপনার ক্যাশফ্লো অনুযায়ী মাসিক পরিকল্পনা — বাজেট না ভেঙে।",
      },
      {
        title: "সম্পূর্ণ আইনি নিরাপত্তা",
        description:
          "যাচাইকৃত দলিল, নিবন্ধিত স্বাক্ষর, কোনো অস্পষ্টতা নেই।",
      },
    ],
    highlights: [
      "৫+ চলমান ও সম্পন্ন প্রকল্প",
      "ফ্ল্যাট, জমি ও বাণিজ্যিক স্পেস",
      "প্রিমিয়াম লোকেশন",
      "৩০+ সুখী পরিবার",
    ],
  },
  {
    slug: "ahbab-real-estate",
    nameEn: "Ahbab Real Estate",
    nameBn: "আহবাব রিয়েল এস্টেট",
    shortBn: "আহবাব রিয়েল এস্টেট",
    tagline: "নির্মাণ সেবা — প্রতিটি ইট, প্রতিটি দেয়াল যত্নে গড়া",
    description:
      "ফ্ল্যাট ও বাড়ি নির্মাণ থেকে সব ধরনের নির্মাণকাজ — নকশা থেকে চাবি পর্যন্ত।",
    longDescription:
      "আহবাব রিয়েল এস্টেটের মাধ্যমে আমরা ফ্ল্যাট, বাড়ি এবং সব ধরনের নির্মাণকাজ সম্পন্ন করি। নকশা থেকে চাবি হস্তান্তর পর্যন্ত — সম্পূর্ণ নির্মাণ সমাধান এক ছাদের নিচে।",
    icon: "Hammer",
    accent: "blue",
    features: [
      {
        title: "আবাসিক ভবন নির্মাণ",
        description:
          "আধুনিক বাড়ি, অ্যাপার্টমেন্ট ও পারিবারিক কমপ্লেক্স নির্মাণ।",
      },
      {
        title: "বাণিজ্যিক নির্মাণ",
        description:
          "অফিস, দোকান এবং মিশ্র-ব্যবহারের ভবন নির্মাণ।",
      },
      {
        title: "রিনোভেশন ও সম্প্রসারণ",
        description:
          "পুরানো ভবন আধুনিকায়ন এবং সম্প্রসারণের কাজ।",
      },
      {
        title: "নকশা থেকে নির্মাণ",
        description:
          "আর্কিটেকচার, স্ট্রাকচারাল ডিজাইন থেকে নির্মাণ — সবকিছু।",
      },
      {
        title: "মান নিয়ন্ত্রণ",
        description:
          "প্রতিটি ধাপে কঠোর গুণগত তদারকি ও অভিজ্ঞ ইঞ্জিনিয়ার।",
      },
    ],
    highlights: [
      "সম্পূর্ণ নির্মাণ সমাধান",
      "অভিজ্ঞ স্থপতি ও ইঞ্জিনিয়ার দল",
      "সময়মতো হস্তান্তর",
      "দীর্ঘস্থায়ী মান",
    ],
  },
  {
    slug: "promise-international",
    nameEn: "Promise International",
    nameBn: "প্রমিজ ইন্টারন্যাশনাল",
    shortBn: "প্রমিজ ইন্টারন্যাশনাল",
    tagline: "সঞ্চয় ও বিনিয়োগ — ব্যাংকের চেয়ে ১০০ গুণ ভালো",
    description:
      "১০,০০০ টাকা থেকে সঞ্চয়, যেকোনো সময় উত্তোলন, এবং বার্ষিক লভ্যাংশ।",
    longDescription:
      "প্রমিজ ইন্টারন্যাশনালের মাধ্যমে আমরা ব্যাংকের চেয়েও উন্নত সঞ্চয় সেবা দিই। ১০,০০০ টাকা থেকে যেকোনো অঙ্ক জমা রাখুন, ব্যাংকের মতো যেকোনো সময় তুলুন, এবং বছর শেষে লেনদেনের ভিত্তিতে লভ্যাংশ পান। ব্যাংক তো সুদ দেয়, আর সময়মতো নিজের টাকা চাইলেও আজকাল পাওয়া যায় না — আমরা সেই সমস্যার সমাধান।",
    icon: "Landmark",
    accent: "indigo",
    features: [
      {
        title: "১০,০০০ টাকা থেকে শুরু",
        description:
          "কম পরিমাণ থেকেই সঞ্চয় শুরু করতে পারবেন — যেকোনো অঙ্ক।",
      },
      {
        title: "যেকোনো সময় উত্তোলন",
        description:
          "ব্যাংকের মতো যখন প্রয়োজন তখনই টাকা তুলতে পারবেন।",
      },
      {
        title: "বার্ষিক লভ্যাংশ",
        description:
          "সারা বছরের লেনদেনের ভিত্তিতে বছর শেষে লভ্যাংশ — সুদ নয়।",
      },
      {
        title: "স্বচ্ছ পরিচালনা",
        description:
          "প্রতিটি লেনদেন স্পষ্ট ও যাচাইযোগ্য, কোনো লুকানো শর্ত নেই।",
      },
      {
        title: "দ্রুত জরুরি সেবা",
        description:
          "জরুরি প্রয়োজনে দ্রুত সাড়া — দীর্ঘ প্রক্রিয়ার ঝামেলা নেই।",
      },
    ],
    highlights: [
      "ব্যাংকের চেয়ে উচ্চতর সুবিধা",
      "নমনীয় ও সহজ সঞ্চয়",
      "লেনদেন-ভিত্তিক লভ্যাংশ",
      "স্বচ্ছ ও বিশ্বস্ত",
    ],
  },
  {
    slug: "ahbab-travels-tours",
    nameEn: "Ahbab Travels & Tours",
    nameBn: "আহবাব ট্রাভেলস অ্যান্ড ট্যুরস",
    shortBn: "আহবাব ট্রাভেলস",
    tagline: "হজ্জ ও উমরাহ — দেশসেরা ও বিশ্বস্ত পরিষেবা",
    description:
      "নির্ভরযোগ্য দেশসেরা হজ্জ ও উমরাহ সেবা — ভিসা থেকে দেশে ফেরা পর্যন্ত।",
    longDescription:
      "আহবাব ট্রাভেলস অ্যান্ড ট্যুরস বাংলাদেশের অন্যতম নির্ভরযোগ্য হজ্জ ও উমরাহ পরিষেবা প্রদানকারী। প্রতিটি ধাপে আপনার পাশে থাকি — ভিসা প্রক্রিয়া থেকে যাত্রা, এবং সফলভাবে দেশে ফেরা পর্যন্ত। অভিজ্ঞ গাইড, প্রিমিয়াম থাকার ব্যবস্থা এবং সম্পূর্ণ সহায়তা।",
    icon: "Plane",
    accent: "amber",
    features: [
      {
        title: "হজ্জ প্যাকেজ",
        description:
          "সম্পূর্ণ হজ্জ প্যাকেজ — ভিসা, ফ্লাইট, হোটেল ও মুয়াল্লিম সেবা।",
      },
      {
        title: "উমরাহ প্যাকেজ",
        description:
          "বছরজুড়ে উমরাহ যাত্রা — বিভিন্ন বাজেট ও মেয়াদের অপশন।",
      },
      {
        title: "ভিসা প্রক্রিয়া",
        description:
          "দ্রুত ও সঠিক ভিসা প্রক্রিয়াকরণ — সব কাগজপত্র আমরা সামলাই।",
      },
      {
        title: "হোটেল ও পরিবহন",
        description:
          "মক্কা ও মদিনায় প্রিমিয়াম থাকা এবং নির্ভরযোগ্য পরিবহন।",
      },
      {
        title: "অভিজ্ঞ গাইড",
        description:
          "জ্ঞানী ও অভিজ্ঞ মুয়াল্লিম এবং বাংলা ভাষায় গাইড।",
      },
      {
        title: "সম্পূর্ণ সহায়তা",
        description:
          "যাত্রার আগে, চলাকালীন ও পরে — ২৪/৭ সহায়তা।",
      },
    ],
    highlights: [
      "দেশসেরা সেবা",
      "শতভাগ স্বচ্ছ মূল্য",
      "অভিজ্ঞ মুয়াল্লিম দল",
      "প্রিমিয়াম থাকা ও পরিবহন",
    ],
  },
  {
    slug: "interior-3d-design",
    nameEn: "Interior & 3D Design",
    nameBn: "ইন্টেরিয়র ও 3D ডিজাইন",
    shortBn: "ইন্টেরিয়র ও 3D",
    tagline: "আপনার স্থাপনা — শিল্প হয়ে উঠুক",
    description:
      "বাড়ি, অফিস, দোকান — যেকোনো স্থাপনার অভ্যন্তরীণ ডিজাইন ও 3D ভিজ্যুয়ালাইজেশন।",
    longDescription:
      "প্রমিজ গ্রুপের ইন্টেরিয়র ও 3D ডিজাইন বিভাগে আমরা আপনার বাড়ি, অফিস, দোকান — যেকোনো স্থাপনার অভ্যন্তরীণ ডিজাইন এবং 3D ভিজ্যুয়ালাইজেশন তৈরি করি। নির্মাণের আগেই দেখুন আপনার স্বপ্ন কেমন দেখাবে।",
    icon: "Palette",
    accent: "rose",
    features: [
      {
        title: "আবাসিক ইন্টেরিয়র",
        description:
          "বসার ঘর, শোবার ঘর, রান্নাঘর — প্রতিটি কোণে নিখুঁত নকশা।",
      },
      {
        title: "অফিস ও কমার্শিয়াল",
        description:
          "অফিস, শো-রুম ও দোকানের জন্য আধুনিক ও কার্যকর নকশা।",
      },
      {
        title: "3D রেন্ডারিং",
        description:
          "ফটোরিয়ালিস্টিক 3D ভিজ্যুয়ালাইজেশন — নির্মাণের আগেই দেখুন।",
      },
      {
        title: "স্পেস প্ল্যানিং",
        description:
          "প্রতিটি বর্গফুটের সর্বোচ্চ ব্যবহার — স্মার্ট লেআউট।",
      },
      {
        title: "ফার্নিচার সিলেকশন",
        description:
          "নকশার সাথে মিলিয়ে সঠিক ফার্নিচার ও উপাদান নির্বাচন।",
      },
      {
        title: "কাস্টম সমাধান",
        description:
          "আপনার রুচি ও বাজেট অনুযায়ী সম্পূর্ণ ব্যক্তিগত ডিজাইন।",
      },
    ],
    highlights: [
      "ফটোরিয়ালিস্টিক 3D",
      "আবাসিক ও কমার্শিয়াল",
      "স্পেস অপ্টিমাইজেশন",
      "ব্যক্তিগত স্পর্শ",
    ],
  },
];

export const NAV = [
  { label: "হোম", href: "/#home", id: "home" },
  { label: "আমাদের সম্পর্কে", href: "/#about", id: "about" },
  {
    label: "আমাদের বিভাগ",
    href: "/#divisions",
    id: "divisions",
    dropdown: DIVISIONS.map((d) => ({
      label: d.nameBn,
      tagline: d.tagline,
      href: `/divisions/${d.slug}`,
      slug: d.slug,
      icon: d.icon,
      accent: d.accent,
    })),
  },
  { label: "প্রকল্প", href: "/#projects", id: "projects" },
  { label: "কেন আমরা", href: "/#why", id: "why" },
  { label: "যোগাযোগ", href: "/#contact", id: "contact" },
];

export const NAV_IDS = ["home", "about", "divisions", "projects", "why", "contact"];

/** Dynamic navbar title + tagline per section */
export const SECTION_META: Record<
  string,
  { title: string; tagline: string }
> = {
  home: { title: "Promise PPD", tagline: SITE.tagline },
  about: { title: "আমাদের সম্পর্কে", tagline: "১৫+ বছরের আস্থা ও অঙ্গীকার" },
  stats: { title: "আমাদের যাত্রা", tagline: "সংখ্যায় প্রমিজ গ্রুপ" },
  divisions: { title: "আমাদের বিভাগ", tagline: "৫টি বিভাগ — এক ছাদের নিচে" },
  projects: { title: "চলমান প্রকল্প", tagline: "ঢাকার বুকে নতুন ঠিকানা" },
  why: { title: "কেন প্রমিজ গ্রুপ", tagline: "পরিবার যে কারণে বেছে নেয়" },
  testimonials: { title: "গ্রাহকদের কণ্ঠ", tagline: "তাঁদের কথায় আমাদের গল্প" },
  contact: { title: "যোগাযোগ করুন", tagline: "এক কর্মদিবসেই উত্তর পাবেন" },
};

export const STATS = [
  { value: 15, suffix: "+", label: "বছরের অভিজ্ঞতা" },
  { value: 5, suffix: "", label: "ব্যবসায়িক বিভাগ" },
  { value: 5, suffix: "+", label: "প্রকল্প" },
  { value: 30, suffix: "+", label: "সুখী পরিবার" },
];

export type Project = {
  name: string;
  status: "চলমান" | "সম্পন্ন" | "আসন্ন";
  location: string;
  price: string;
  size?: string;
  description: string;
  highlights: string[];
};

export const PROJECTS: Project[] = [
  {
    name: "ফুজলা টাওয়ার",
    status: "চলমান",
    location: "প্রমিজ সিটি, ঢাকা",
    price: "৳ ৮.৫০ লাখ",
    description:
      "প্রমিজ সিটির প্রাণকেন্দ্রে আধুনিক আবাসিক টাওয়ার — প্রিমিয়াম অ্যামেনিটি ও স্কাইলাইন ভিউ সহ।",
    highlights: ["প্রিমিয়াম লোকেশন", "স্কাইলাইন ভিউ", "আধুনিক সুবিধা"],
  },
  {
    name: "ফুজলা কমপ্লেক্স",
    status: "চলমান",
    location: "প্রমিজ সিটি, ঢাকা",
    price: "৳ ৪.৪৫ লাখ",
    description:
      "সাশ্রয়ী মূল্যের পারিবারিক কমপ্লেক্স — চিন্তাশীল লেআউট, নিরাপদ কমিউনিটি ও সহজ যাতায়াত।",
    highlights: ["পারিবারিক ডিজাইন", "নিরাপদ কমিউনিটি", "সহজ যাতায়াত"],
  },
  {
    name: "আহবাব প্যালেস · ০১",
    status: "চলমান",
    location: "ঢাকা",
    price: "৳ ৪০.০০ লাখ",
    description:
      "ফ্ল্যাগশিপ প্যালেস-ক্লাস বাসস্থান — প্রশস্ত যাপন, প্রিমিয়াম ফিনিশিং ও নিরবচ্ছিন্ন গোপনীয়তা।",
    highlights: ["ফ্ল্যাগশিপ প্রকল্প", "প্রিমিয়াম ফিনিশিং", "একান্ত গোপনীয়তা"],
  },
  {
    name: "আহবাব প্যালেস · ০২",
    status: "চলমান",
    location: "ঢাকা",
    price: "৳ ১৪.৫০ লাখ",
    size: "১২০০ বর্গফুট",
    description:
      "কমপ্যাক্ট বিলাসিতা ১২০০ বর্গফুটে — খোলা রান্নাঘর, ব্যালকনি ও আধুনিক ফিটিং।",
    highlights: ["১২০০ বর্গফুট", "খোলা রান্নাঘর", "আধুনিক ফিটিং"],
  },
  {
    name: "আহবাব প্যালেস · ০২",
    status: "চলমান",
    location: "ঢাকা",
    price: "৳ ২১.৭৫ লাখ",
    size: "১৮০০ বর্গফুট",
    description:
      "১৮০০ বর্গফুটের প্রশস্ত বাসস্থান — মাস্টার স্যুট, বড় ব্যালকনি ও পরিবারের জন্য আদর্শ।",
    highlights: ["১৮০০ বর্গফুট", "মাস্টার স্যুট", "বড় ব্যালকনি"],
  },
];

export const WHY_US = [
  {
    title: "প্রিমিয়াম লোকেশন",
    description: "ঢাকার বিকাশমান এলাকা থেকে হাতে বাছাই করা প্রতিটি প্রকল্প।",
    icon: "MapPin",
  },
  {
    title: "গুণগত নিশ্চয়তা",
    description: "প্রতিটি দেয়াল, প্রতিটি ফিটিং — দশকের পর দশক টিকে থাকার মতো।",
    icon: "Award",
  },
  {
    title: "১৫+ বছরের অভিজ্ঞতা",
    description: "২০১০ সাল থেকে পরিবারগুলোকে সঠিক পথ দেখাচ্ছি, সঠিকভাবে।",
    icon: "Users",
  },
  {
    title: "নমনীয় কিস্তি",
    description: "আপনার বেতনের সাথে চলে — বিরুদ্ধে নয়। এমন পরিকল্পনা।",
    icon: "CreditCard",
  },
  {
    title: "আইনি নিরাপত্তা",
    description: "যাচাইকৃত দলিল, নিবন্ধিত স্বাক্ষর, কোনো অস্পষ্টতা নেই।",
    icon: "ShieldCheck",
  },
  {
    title: "৫ বিভাগ এক ছাদের নিচে",
    description: "রিয়েল এস্টেট, নির্মাণ, সঞ্চয়, হজ্জ ও ডিজাইন — সব একসাথে।",
    icon: "Layers",
  },
];

export const TESTIMONIALS = [
  {
    name: "রাশেদ আহমেদ",
    role: "গৃহকর্তা · ফুজলা টাওয়ার",
    quote:
      "প্রথম সাইট ভিজিট থেকে চাবি হাতে পাওয়ার দিন পর্যন্ত প্রতিটি ধাপ সহজ মনে হয়েছে। সৎ মানুষ, সৎ কাজ।",
  },
  {
    name: "নুসরাত জাহান",
    role: "পরিবার · আহবাব প্যালেস",
    quote:
      "ঢাকার ছয়জন ডেভেলপারের সাথে তুলনা করেছি। স্বচ্ছতাতেই প্রমিজ গ্রুপ জিতেছে — আর নির্মাণের মানও প্রতিশ্রুতির চেয়েও ভালো।",
  },
  {
    name: "আবদুল হালিম",
    role: "হাজী · আহবাব ট্রাভেলস",
    quote:
      "আল্লাহর ঘরে যাত্রার প্রতিটি ধাপে আহবাব ট্রাভেলস পাশে ছিল। নির্ভরযোগ্যতা, যত্ন এবং ইসলামি আদব — সবকিছু পেয়েছি।",
  },
  {
    name: "ইমরান হোসেন",
    role: "সঞ্চয়ী · প্রমিজ ইন্টারন্যাশনাল",
    quote:
      "ব্যাংকের ঝামেলা ছাড়াই প্রতিমাসে সঞ্চয় করছি, যখন প্রয়োজন তখনই তুলেছি। স্বচ্ছতা ও সুবিধা — দুটোই।",
  },
  {
    name: "সাবিনা ইয়াসমিন",
    role: "গৃহকর্তা · ইন্টেরিয়র ডিজাইন",
    quote:
      "3D ডিজাইন দেখে নির্মাণের আগেই বাড়িটা মনে হয়েছিল চিনি। ফিনিশিং-ও ঠিক তেমনই হয়েছে।",
  },
];

export const INTERESTS = [
  "প্রমিজ সিটি — ফ্ল্যাট/জমি",
  "আহবাব রিয়েল এস্টেট — নির্মাণ",
  "প্রমিজ ইন্টারন্যাশনাল — সঞ্চয়",
  "আহবাব ট্রাভেলস — হজ্জ/উমরাহ",
  "ইন্টেরিয়র ও 3D ডিজাইন",
  "সাধারণ পরামর্শ",
];
